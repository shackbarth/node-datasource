...
// Not working perfectly
    ...
    for (i in line) {
        ...
        for (j in bytes) {
            ...
            if (i === "1") {
              console.log(bytes[j]);
              console.log(bytes[j].charCodeAt(0));
              console.log(tmp1);
            }
            if (tmp1 === 64) {
              // some corner case
              tmp1 = 0;
              console.log("Crazy conversion");
            }
...
convert_uuencode: function (str) {
  ...
  while (chunk() !== 0) {
    ...
    for (i = 0; i <= (tmp2.length / 6) - 1; i++) {
      ...
      if (tmp1 == "000000") {
        console.log(tmp1 + " " + "special " + c + " " + String.fromCharCode(96));
        encoded += chr(96);
      } else {
        console.log(tmp1 + " " + (parseInt(tmp1, 2) + 32) + " " + String.fromCharCode(parseInt(tmp1, 2) + 32));
